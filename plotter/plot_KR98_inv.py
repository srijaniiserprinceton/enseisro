...
Npoints = 100                                                                                                                           
Prot = np.logspace(np.log10(1), np.log10(28), Npoints)
...
for i, Prot_low in enumerate(Prot_low_arr):
    ...
    DOmega_by_Omega_avg, err_DOmega_by_Omega_avg = FN.propagate_errors(Omega_avg, DOmega, err_Omega_avg, err_DOmega)
    
    # plotting the inverted value with error bars
    for j, multiply_star in enumerate(multiply_star_arr):
        fac = multiply_star_arr[j]
        ax.errorbar(np.mean(Prot), DOmega_by_Omega_avg, yerr=err_DOmega_by_Omega_avg/np.sqrt(fac),\
                                            fmt = 'o', capsize=10, label='%.0e'%(Nstars*fac))
